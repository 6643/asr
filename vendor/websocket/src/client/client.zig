...
pub const Client = struct {
    ...
    fn readLoopOwnedThread(self: *Client, h: anytype) void {
        const Handler = ReadLoopHandler(@TypeOf(h));
        self.readLoop(h) catch |err| {
            if (comptime std.meta.hasFn(Handler, "readLoopError")) {
                h.readLoopError(err);
            }
        };
    }
    ...
    pub fn close(self: *Client, opts: CloseOpts) !void {
        ...
        self._write_mutex.lockUncancelable(self.io);
        defer self._write_mutex.unlock(self.io);
        defer self.closeStreamLocked();

        const code = opts.code orelse {
            self.writeFrameLocked(.close, "") catch {};
            return;
        };
        ...
        self.writeFrameLocked(.close, buf[0..end]) catch {};
    }

    pub fn writeFrame(self: *Client, op_code: proto.OpCode, data: []u8) !void {
        self._write_mutex.lockUncancelable(self.io);
        defer self._write_mutex.unlock(self.io);
        try self.writeFrameLocked(op_code, data);
    }

    fn writeFrameLocked(self: *Client, op_code: proto.OpCode, data: []u8) !void {
        if (@atomicLoad(bool, &self._stream_closed, .monotonic)) return error.Closed;
        const payload = data;
    ...
    fn closeStream(self: *Client) void {
        self._write_mutex.lockUncancelable(self.io);
        defer self._write_mutex.unlock(self.io);
        self.closeStreamLocked();
    }

    fn closeStreamLocked(self: *Client) void {
        if (@atomicRmw(bool, &self._stream_closed, .Xchg, true, .monotonic) == false) {
            self.stream.close();
        }
    }
};
